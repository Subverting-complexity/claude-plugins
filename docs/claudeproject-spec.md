...
**Optional** — remove if unused: `## Project Board`, `## Reference Docs`, `## Bundled Skills`, the `### Custom` label table.
...
- **Missing recommended/optional content** — WARNING at most; commands proceed on defaults (`templates/default-labels.md` for labels, `main` for the default branch, `feature/{number}/{short-desc}` for branches).
