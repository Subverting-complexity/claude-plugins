...
**Required** — preflight emits a CRITICAL (blocking, offers the setup wizard) if the literal level-2 heading is absent:
...
**Recommended** — read by commands, default-covered when absent: `## Story Template`, `## Session Budget`, `## Ready Gate`, `## Agent Gating`, `## Refinement`, `## Issue Types & Fields`.
...
- **Label map rows**: kept only when the purpose key matches `^[a-z]+-[a-z-]+$` (e.g. `priority-high`, `status-ready`). Unmapped purposes resolve to defaults in `templates/default-labels.md` (WARNING, never blocking).
- **Ready gate**: `ready-gate` row value — `label` (default), `board-column`, `both`, or `none` (`off`/`disabled` normalise to `none`). `board-column`/`both` make a board **required**.
...
- **Missing recommended/optional content** — WARNING at most; commands proceed on defaults (`templates/default-labels.md` for labels and columns, `main` for the default branch, `feature/{number}/{short-desc}` for branches, ready-gate `label`, agent-gating `disabled`).
- **Board section absent or unparseable** — treated as "no board": board moves are skipped silently. But a board that **is** configured while missing any of the three active lifecycle columns (`col-in-progress`, `col-in-review`, `col-blocked`) raises CRITICAL `board-columns-incomplete`; a required board (ready-gate `board-column`/`both`) that is absent or whose `project-node-id` no longer resolves to the recorded `project-title` is CRITICAL too.
