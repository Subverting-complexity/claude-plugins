...
**Configured for `github-workflow`**, verified by a committed `ClaudeProject.md`. The plugin is driven there from a per-machine install, so nothing in the repository records the dependency, but a breaking release affects each of them:
