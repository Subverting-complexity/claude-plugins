...
**Why `bypass-ci-when-no-pipeline: false`.** This one is not a judgement call — it cannot apply here. It only ever engages on a repo with **zero** active GitHub Actions workflows, and this repo has two (`CI` and `Copilot`), so an empty check rollup on a PR here would mean something has gone wrong rather than that there is nothing to report. The setting exists for projects whose CI is permanently invisible to GitHub — no pipeline, or one running on Buildkite/Jenkins/CircleCI that never posts a status back — where without it every approved PR pauses at the no-checks guard forever.
