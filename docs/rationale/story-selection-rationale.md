...
Under `ready-gate: none` every open unassigned issue is eligible, but a `status-blocked` issue is unassigned (so `--assignee ""` does not exclude it) yet has an unresolved blocker. Including it would just claim, re-check, re-block, and waste calls each pass. The other non-pickable states need no special handling: `status-parked` / `status-in-progress` / `status-in-review` stay assigned (already excluded), and `needs-refinement` is dropped by the refinement filter. A `status-blocked` issue whose dependencies have actually closed is restored to `status-ready` by Step 4 on a later pass.
