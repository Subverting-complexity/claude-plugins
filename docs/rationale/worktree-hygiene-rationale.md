...
> Start clean → so everything dirty at the end was produced by **this** session → commit it (feature + a `chore:` formatting commit) → end clean → the harness reaps the worktree.

Because the worktree starts clean, there is no "foreign work I didn't author" case at exit to agonize over: inherited dirt is dealt with once, at the start. Everything still dirty at exit is yours, so it is either committed or discarded as disposable noise — never left loose.
