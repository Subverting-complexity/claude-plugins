...
| `bulk-execute` (github) | github-only, and deliberately not a flag on `execute`. It needs GitHub issues, claims, org issue fields and one PR closing several issues, none of which local-workflow has. It reuses `execute`'s review, merge and cleanup references rather than copying them |
...
| `preflight` | github: issue field/label/auth validator; local: lightweight git/config checks |
