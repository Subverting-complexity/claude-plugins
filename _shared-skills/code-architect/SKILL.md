...
1. **Establish shared understanding** - Call `feature-discovery` in validation mode to clarify (skip this step when called from an autonomous workflow like `/{{PLUGIN_NAME}}:{{EXECUTE_SKILL}}` that explicitly says not to pause or call feature-discovery):
