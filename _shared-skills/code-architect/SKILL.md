...
1. **Establish shared understanding** - Run the `grill` skill (`/{{PLUGIN_NAME}}:grill`) to clarify (skip this step when called from an autonomous workflow like `/{{PLUGIN_NAME}}:{{EXECUTE_SKILL}}` that explicitly says not to pause or interview the user):
