...
1. **Establish shared understanding** - Run the `grill` skill (`/local-workflow:grill`) to clarify (skip this step when called from an autonomous workflow like `/local-workflow:build` that explicitly says not to pause or interview the user):
