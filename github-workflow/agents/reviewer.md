...
- For anything that needs human judgment (architectural decisions, ambiguous requirements) — do not guess. Flag it under "Issues remaining" with a `changes-requested` or `needs-discussion` verdict **and** file it to the board with `/github-workflow:report-issue` (correct type) so it is picked up automatically. The same applies to any non-blocking issue, conflict, or failing check you cannot fix in place: file it to the board rather than dropping it or pausing for a human. No human approval is needed.
