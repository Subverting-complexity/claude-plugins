...
Read by `/github-workflow:setup` with no argument, after Step 1 (prerequisites) in the command. Run every step in order. Each step reads its own template or guide at the point it runs, not before.
...
Suggest running `/github-workflow:execute` to start the first story. Mention `/github-workflow:setup reap` for a story or PR that is stuck behind a crashed session's claim, and `/github-workflow:setup issues` to backfill issues created outside the workflow.
