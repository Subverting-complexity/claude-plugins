...
Read by `/github-workflow:setup reap`. Read `ClaudeProject.md` for org and repo first.
...
It is safe to run at any time, because it never reaps a ref that still backs a running session, and is schedulable via `/schedule` calling `/github-workflow:setup reap`.
