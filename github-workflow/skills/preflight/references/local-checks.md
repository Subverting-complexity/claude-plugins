...
For a project with no `ClaudeProject.md`, which is local work rather than GitHub story work, these checks replace `wf preflight`. They validate the local setup before starting work, and they are **read-only**: it checks and reports, and never creates, edits, or deletes anything — no files, no branches, no config.
...
Summarize the output as a short pass/warn list — `[pass]` items first, briefly, then one line per `WARN` with exactly one actionable next step:
...
- `ecosystem` → optional: run `/github-workflow:ecosystem-setup`, or skip
- `quality-gate` → tell Claude the test command, or record it in `CLAUDE.md`

Warnings never block — report them and stop. Do not fix anything, even if the fix looks trivial; the user decides what happens next.

The **shape** of that report follows `skills/user-facing-communication/SKILL.md`: the overall verdict first, then the warnings. Say plainly that nothing was changed, because a read-only check is easy to mistake for a fix.
