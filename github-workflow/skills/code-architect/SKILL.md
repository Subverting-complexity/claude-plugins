...
1. **Establish shared understanding** - Run the `grill` skill (`/github-workflow:grill`) to clarify (skip this step when called from an autonomous workflow like `/github-workflow:execute` that explicitly says not to pause or interview the user):
