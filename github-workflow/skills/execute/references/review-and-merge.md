...
2. **File only the two things the rubric says to file:** a defect in pre-existing code that neither the diff touches nor the story covers, and a question that genuinely needs human judgment. File either with `/github-workflow:report-issue` (autonomous, no lifecycle label, correct type, referencing this PR and the `file:line`). The second kind also holds the PR: see the Needs Discussion rule below. Do not guess at it, and do not drop it.
