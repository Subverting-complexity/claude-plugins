...
1. If it printed "ClaudeProject.md NOT FOUND", stop with exactly one message, "ClaudeProject.md not found — run /github-workflow:setup.", and do not chain into preflight for the same cause.
2. Run `test -f .claude/preflight-passed.txt && echo "PREFLIGHT_ALREADY_PASSED"`. The file is written by a clean or WARNING-only preflight and deleted by **Exit cleanup**, so it is valid for this session. If it is absent, invoke `/github-workflow:preflight`. On "Configure now", wait for setup and ask the user to re-run the command, because the loaded configuration is stale. On "Continue anyway" or "Don't remind me", proceed.
3. The projection must contain both `## Identity` and `## Quality Gate`. If either is missing, stop with "ClaudeProject.md is missing required section: {name} — run /github-workflow:setup."
...
- **Anywhere else** — a pre-existing bug in untouched code, a problem noticed in passing, work belonging to a story this run does not close — file it with `/github-workflow:report-issue` and carry on. Fixing it inline widens the diff the reviewer has to judge.
