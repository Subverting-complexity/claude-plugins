...
Read this file only when `$ARGUMENTS.mode` is `audit`. Audit is a
no-code-change pass over the codebase that files issues for findings; it
applies to a minority of runs, so it is kept out of the main `SKILL.md`.
