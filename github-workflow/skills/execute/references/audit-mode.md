...
1. Read `ClaudeProject.md` for org, repo, and the field names under Issue Types & Fields.
2. Audit the default branch — read the codebase structure, key files, and patterns. Check for architecture violations, security issues, test gaps, dead code, and tech debt. Use the evaluation criteria from the pr-review skill (non-compliance gates, correctness, security, test coverage) but apply them to the codebase at large, not to a specific PR diff.
...
3. For each finding, run `/github-workflow:report-issue` to create a GitHub issue. It sets the native issue type and the `Priority`, `Effort` and `Ownership` fields; you supply the judgement those need, and no label carries any of it. Cap at 10 issues per audit session to keep scope manageable.
