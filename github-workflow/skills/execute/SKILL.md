...
5. If any gaps are found, leave a self-comment on the PR:
   ```
   gh pr comment {pr_number} --repo {org}/{repo} --body "## Self-review notes

   The following items may need attention before merge:

   {numbered list of gaps found}

   _Auto-generated by github-workflow execute self-review_"
   ```
