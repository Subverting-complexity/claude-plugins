...
- The story needs breaking into sub-stories — run `/github-workflow:feature-discovery` to plan the breakdown with the user, then pick the first sub-story.
...
**Interactive discovery gate.** When a user is present and mode is `story` or `feature`, run `/github-workflow:grill` on the story's requirements before planning. Skip it in an autonomous session, in `maintenance` or `audit` mode, or when the issue body already has discovery output (`## Stories` or `## Architecture`).

## Phase 3 — Plan

Use `/github-workflow:code-architect` to plan the implementation, passing the issue requirements, the relevant codebase context, and any reference docs listed in `ClaudeProject.md`. The plan file and ecosystem tools are in the shared rules.
