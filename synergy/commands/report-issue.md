---
description: 'File a bug, security, architecture or tech debt issue. Trigger: "report a bug", "create an issue".'
---
...
Before doing anything else, check whether preflight already passed in the last four hours — an autonomous filing from `execute`, `pr-review` or audit mode calls this command repeatedly in one run, and the check is what keeps a repeat filing from loading the whole skill again:
