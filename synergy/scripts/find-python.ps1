...
#
# Candidate order: the Windows Python Launcher (`py -3`), then `python3`, then `python`. Each candidate must actually run `--version`, not just be on PATH, so a broken shim (the Microsoft Store python3 stub) is skipped.
#
...
function Find-Python {
    foreach ($candidate in @(
        ...
        try { $ver = "$(& $candidate.Command @versionArgs 2>&1)".Trim() } catch { continue }
