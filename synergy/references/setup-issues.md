...
By default the audit proposes no dependency edges, because a native blocked-by edge is the only record a dependency has, so it cannot disagree with anything. It reads two things out of a body, each only when asked: the parent an issue claims (`--parents`), and a `Blocked by: #N` line naming an open issue the edges do not include (`--blockers`, run against the whole backlog, which proposes the `blocked_by` list beside the other gaps). `--blockers-only` writes a spec of just those edges with no `TODO`, so `issue-apply` can take it as it stands. To add or remove an edge, put a `blocked_by` list on the spec entry yourself. It is the complete set, so the edges it names are added and any the issue carries that it omits are removed.
