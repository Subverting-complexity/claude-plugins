...
> 3. **Stay private + Free**: configuration (a) is impossible, so **(b) is your only gate.** This is not a stopgap for these repos; it is the enforcement mechanism. `/synergy:onboard harden` detects the `403` and sets `require-ci-before-merge: true` automatically.
>
