...
- When Phase 10 merges, it reads `skills/execute/references/merge.md` unchanged. Its release notes are written per story, from that story's own commits, so each issue carries its own text. Its settle step, `wf post-merge --pr {pr_number} --notes .claude/release-notes.json`, already closes and moves **every** issue the pull request closes; report each settled story by number and title, and the `unblocked` sweep, because a bulk set is the run most likely to free downstream work.
