...
Gating on that one setting is safe because it is the only thing that enables a merge anywhere in the plugin: it governs both `/synergy:pr-review`'s merge step and the merge phase that ends a `/synergy:execute` run. There is no second path that merges without it, so a project this block skips is a project where nothing merges unattended.
