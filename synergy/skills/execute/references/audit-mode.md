...
3. For each finding, run `/synergy:report-issue` to create a GitHub issue. It sets the native issue type and the `Priority`, `Effort` and `Ownership` fields; you supply the judgement those need, and no label carries any of it. Cap at 10 issues per audit session to keep scope manageable.
